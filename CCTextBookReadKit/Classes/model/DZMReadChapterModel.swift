...
public class DZMReadChapterModel: NSObject,NSCoding {
    ...
    /// 上一章ID
    public var previousChapterID:NSNumber!
    
    /// 下一章ID
    public var nextChapterID:NSNumber!
   ...
   public  var priority:NSNumber!
    ...
    /// 本章有多少页
    public var pageCount:NSNumber! = NSNumber(value: 0)
    
    /// 分页数据
    public var pageModels:[DZMReadPageModel]! = []
    
    
    // MARK: 快捷获取
    
    /// 当前章节是否为第一个章节
    public var isFirstChapter:Bool! { return (previousChapterID == DZM_READ_NO_MORE_CHAPTER) }
    
    /// 当前章节是否为最后一个章节
    public var isLastChapter:Bool! { return (nextChapterID == DZM_READ_NO_MORE_CHAPTER) }
    
    /// 完整章节名称
    public var fullName:String! { return DZM_READ_CHAPTER_NAME(name) }
    
    /// 完整富文本内容
   public  var fullContent:NSAttributedString!
    
    /// 分页总高 (上下滚动模式使用)
    public var pageTotalHeight:CGFloat {
    ...
    /// 获取指定页码字符串
    public func contentString(page:NSInteger) ->String {
        
        return pageModels[page].content.string
    }

    /// 获取指定页码富文本
    public func contentAttributedString(page:NSInteger) ->NSAttributedString {
        
        return pageModels[page].showContent
    }
    
    /// 获取指定页开始坐标
    public func locationFirst(page:NSInteger) ->NSNumber {
        
        return NSNumber(value: pageModels[page].range.location)
    }
    
    /// 获取指定页码末尾坐标
    public func locationLast(page:NSInteger) ->NSNumber {
    ...
    /// 获取指定页中间
    public func locationCenter(page:NSInteger) ->NSNumber {
    ...
    /// 获取存在指定坐标的页码
    public func page(location:NSInteger) ->NSNumber {
    ...
    /// 保存
    public func save() { DZMKeyedArchiver.archiver(folderName: bookID, fileName: id.stringValue, object: self) }
    ...
    /// 获取章节对象,如果则创建对象返回
    public class func model(bookID:String!, chapterID:NSNumber!, isUpdateFont:Bool = true) ->DZMReadChapterModel {
