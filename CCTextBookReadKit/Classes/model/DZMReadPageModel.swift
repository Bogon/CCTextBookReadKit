...
public class DZMReadPageModel: NSObject,NSCoding {
    ...
    /// 当前页序号
    public var page:NSNumber!
    
    
    // MARK: 滚动模式使用
    
    /// 根据开头类型返回开头高度 (目前主要是滚动模式使用)
    public var headTypeHeight:CGFloat! = 0
    
    /// 当前内容Size (目前主要是(滚动模式 || 长按模式)使用)
    public var contentSize:CGSize! = CGSize.zero
    
    /// 当前内容头部类型 (目前主要是滚动模式使用)
    public var headTypeIndex:NSNumber!
    
    /// 当前内容头部类型 (目前主要是滚动模式使用)
    public var headType:DZMPageHeadType! {
    ...
    /// 书籍首页
    public var isHomePage:Bool { return range.location == DZM_READ_BOOK_HOME_PAGE }
    
    /// 获取显示内容(考虑可能会变换字体颜色的情况)
    public var showContent:NSAttributedString! {
