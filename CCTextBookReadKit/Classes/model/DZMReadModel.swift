...
public class DZMReadModel: NSObject,NSCoding {
    ...
    /// 小说来源类型
    public var bookSourceType:DZMBookSourceType! = .network
    
    /// 当前阅读记录
    public var recordModel:DZMReadRecordModel!
    
    /// 书签列表
    public var markModels:[DZMReadMarkModel]! = []
    
    /// 章节列表(如果是网络小说可以不需要放在这里记录,直接在目录视图里面加载接口或者读取本地数据库就好了。)
    public var chapterListModels:[DZMReadChapterListModel]! = []
    ...
    public var ranges:[String:[String:NSRange]]!
    ...
    /// 获取阅读对象,如果则创建对象返回
   public  class func model(bookID:String!) ->DZMReadModel {
