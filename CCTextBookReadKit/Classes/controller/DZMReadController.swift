...
public class DZMReadController: DZMViewController,DZMReadMenuDelegate,UIPageViewControllerDelegate,UIPageViewControllerDataSource,CCBRKCoverControllerDelegate,DZMReadContentViewDelegate,DZMReadCatalogViewDelegate,DZMReadMarkViewDelegate {

    // MARK: 数据相关
    
    /// 阅读对象
    public var readModel:DZMReadModel!
    
    
    // MARK: UI相关
    
    /// 阅读主视图
    public var contentView:DZMReadContentView!
    
    /// 章节列表
    public var leftView:DZMReadLeftView!
    
    /// 阅读菜单
    public var readMenu:DZMReadMenu!
    
    /// 翻页控制器 (仿真)
    public var pageViewController:UIPageViewController!
    
    /// 翻页控制器 (滚动)
    public var scrollController:DZMReadViewScrollController!
    
    /// 翻页控制器 (无效果,覆盖)
    public var coverController:CCBRKCoverController!
    
    /// 非滚动模式时,当前显示 DZMReadViewController
    public var currentDisplayController:DZMReadViewController?
    
    /// 用于区分正反面的值(勿动)
    public var tempNumber:NSInteger = 1
