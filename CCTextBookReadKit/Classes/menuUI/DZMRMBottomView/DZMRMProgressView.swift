...
import UIKit
import CCBRKValuePopUpView

class DZMRMProgressView: DZMRMBaseView,CCBRKValueTrackingSliderDelegate,CCBRKValueTrackingSliderDataSource {
    
    /// 上一章
    private var previousChapter:UIButton!
    
    /// 进度
    private var slider:CCBRKValueTrackingSlider!
    ...
    override func addSubviews() {
        ...
        // 进度条
        slider = CCBRKValueTrackingSlider()
        slider.delegate = self
        slider.dataSource = self
        slider.setThumbImage(UIImage(named:"\(IconPath(WithIconName: "slider"))")!.withRenderingMode(.alwaysTemplate), for: .normal)
        ...
        // 设置气泡字体以及字体大小
        slider.font = UIFont(name: "\(IconPath(WithIconName: "Futura-CondensedExtraBold"))", size: 22)
        // 设置气泡箭头高度
    ...
    func slider(_ slider: CCBRKValueTrackingSlider!, stringForValue value: Float) -> String! {
    ...
    /// 进度显示将要显示
    func sliderWillDisplayPopUpView(_ slider: CCBRKValueTrackingSlider!) { }

    /// 进度显示将要隐藏
    func sliderWillHidePopUpView(_ slider: CCBRKValueTrackingSlider!) {
