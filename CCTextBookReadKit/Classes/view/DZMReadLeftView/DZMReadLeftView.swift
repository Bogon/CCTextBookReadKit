...
public class DZMReadLeftView: UIView,CCBRKSegmentedControlDelegate {

    // 功能栏
    private var segmentedControl:CCBRKSegmentedControl!
    ...
    private func addSubviews() {
        ...
        // 工具栏
        segmentedControl = CCBRKSegmentedControl(frame: CGRect(x: 0, y: SA(isX: StatusBarHeight, 0), width: DZM_READ_LEFT_VIEW_WIDTH, height: DZM_READ_LEFT_HEADER_VIEW_HEIGHT))
    ...
    private func segmentedControl(_ segmentedControl: CCBRKSegmentedControl, click index: Int) {
        
        UIView.animate(withDuration: DZM_READ_AD_TIME) { [weak self] () in
            
            if index == 0 { // 显示目录
                
                self?.bringSubview(toFront: self!.catalogView)
                self?.catalogView.alpha = 1
                self?.markView.alpha = 0
                
            }else{ // 显示书签
                
                self?.bringSubview(toFront: self!.markView)
